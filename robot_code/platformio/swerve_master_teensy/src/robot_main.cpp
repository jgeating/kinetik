...
void robotSetup() {
    ...
    planner = new Planner(&kin, &traj, &profiles, &modes, &loopTiming);
    pads = new Pads(&padVars);
    
    // Initialize PID controllers
    padx_pid = new PID(0.5, 0.0, 0.1, 4500); // Example PID values
    pady_pid = new PID(0.5, 0.0, 0.1, 4500);
    padz_pid = new PID(0.5, 0.0, 0.1, 4500);
...
void robotLoop() {
    ...
    // Update timing
    loopTiming.loopTime = currentTime - lastLoopTime;
    ...
    robotState.estop = signalLost || (redSwitch == 0);
    ...
    robotState.yaw = eulerAngles.z;
    robotState.yawRate = angularVel.z;
    ...
    if (currentTime - lastTelemetryTime > TELEMETRY_REPORT_PERIOD) {
        ...
        HAL::serial->print(robotState.yaw, 2);
        HAL::serial->print(", Vx: ");
        HAL::serial->print(traj.vx, 2);
        HAL::serial->print(", Vy: ");
        HAL::serial->print(traj.vy, 2);
        HAL::serial->print(", Omega: ");
        HAL::serial->print(traj.omega, 2);
        HAL::serial->print(", E-stop: ");
        HAL::serial->println(robotState.estop ? "ACTIVE" : "CLEAR");
